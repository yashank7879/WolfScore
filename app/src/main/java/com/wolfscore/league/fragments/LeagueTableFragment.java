...
import static com.wolfscore.utils.ApiCollection.GET_LEAGUE_TABLE_STATISTICS;
...
*/
    ...
    private void getStatsData() {
        if (Constant.isNetworkAvailable(context, binding.mainLayout)) {
              progressDialog.show();
            AndroidNetworking.get(BASE_URL + GET_LEAGUE_TABLE_STATISTICS)
                    .addHeaders("Api-Key", APIKEY)
                    .addHeaders("Auth-Token", PreferenceConnector.readString(context, PreferenceConnector.AUTH_TOKEN, ""))
                    .addQueryParameter("league_id", ""+AboutLeagueActivity.aboutLeagueActivity.league.getLeague_id())
                    .addQueryParameter("type", "standing")
